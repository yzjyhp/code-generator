...
import com.yzjyhp.generator.engine.AbstractTemplateEngine;
...
public class AutoGenerator {
    ...
    protected ConfigBuilder pretreatmentConfigBuilder(ConfigBuilder config) {
        ...
        boolean v3 = config.getGlobalConfig().getMyBatisPlusVersion() == 3;
        for (TableInfo tableInfo : tableList) {
            /* ---------- 添加导入包 ---------- */
            if (config.getGlobalConfig().isActiveRecord()) {
                // 开启 ActiveRecord 模式
                tableInfo.setImportPackages(MyBatisPlusPackageVal.Model());
            }
            if (tableInfo.isConvert()) {
                // 表注解
                tableInfo.setImportPackages(MyBatisPlusPackageVal.TableName());
            }
            if (tableInfo.isLogicDelete(config.getStrategyConfig().getLogicDeleteFieldName())) {
                // 逻辑删除注解
                tableInfo.setImportPackages(MyBatisPlusPackageVal.TableLogic());
            }
            if (GeneratorStringUtils.isNotEmpty(config.getStrategyConfig().getVersionFieldName())) {
                // 乐观锁注解
                tableInfo.setImportPackages(MyBatisPlusPackageVal.Version());
            }
            if (GeneratorStringUtils.isNotEmpty(config.getSuperEntityClass())) {
                // 父实体
                    ...
                    if ("boolean".equalsIgnoreCase(field.getPropertyType())) {
                        if (field.getPropertyName().startsWith("is")) {
                            field.setPropertyName(config.getStrategyConfig(),
                                    GeneratorStringUtils.removePrefixAfterPrefixToLower(field.getPropertyName(), 2));
                        }
                    }
                }
            }
