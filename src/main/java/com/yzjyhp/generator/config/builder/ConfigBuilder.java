 ...
 * the License.
 */
package com.yzjyhp.generator.config.builder;

import com.yzjyhp.generator.InjectionConfig;
...
import com.yzjyhp.generator.config.MyBatisPlusPackageValV2;
import com.yzjyhp.generator.config.MyBatisPlusPackageValV3;
...
import com.yzjyhp.generator.config.po.TableFill;
...
import com.yzjyhp.generator.config.rules.QuerySQL;
...
public class ConfigBuilder {
     ...
     * Mapper类上面的通用引用
     */
    private List<String> mapperClassPackage;
    /**
     * Service类上面的通用引用
     */
    private List<String> serviceClassPackage;
    /**
     * ServiceImpl类上面的通用引用
     */
    private List<String> serviceImplClassPackage;
    /**
     * service超类定义
     */
    ...
    public List<String> getServiceClassPackage() {
        return serviceClassPackage;
    }

    public void setServiceClassPackage(List<String> serviceClassPackage) {
        this.serviceClassPackage = serviceClassPackage;
    }

    public List<String> getServiceImplClassPackage() {
        return serviceImplClassPackage;
    }

    public void setServiceImplClassPackage(List<String> serviceImplClassPackage) {
        this.serviceImplClassPackage = serviceImplClassPackage;
    }

    public List<String> getMapperClassPackage() {
        return mapperClassPackage;
    }

    public void setMapperClassPackage(List<String> mapperClassPackage) {
    ...
    private void handlerPackage(TemplateConfig template, GlobalConfig globalConfig, PackageConfig config) {
        ...
        String outputXmlDir = globalConfig.getOutputXmlDir();
        String outputClientDir = globalConfig.getOutputClientDir();

        packageInfo = new HashMap<>();
        //初始化对应的类的JAR地址
        packageInfo.put(ConstVal.APPLICATIONNAME, globalConfig.getApplicationName());
        packageInfo.put(ConstVal.MODULENAME, config.getModuleName());
        packageInfo.put(ConstVal.ENTITY, joinPackage(config.getParent(), config.getEntity()));
        packageInfo.put(ConstVal.CLIENT, joinPackage(config.getParent(), config.getClient()));
        packageInfo.put(ConstVal.MODEL, joinPackage(config.getParent(), config.getVo()));
        packageInfo.put(ConstVal.PARAMS, joinPackage(config.getParent(), config.getVo()));
        packageInfo.put(ConstVal.ADD_PARAMS, joinPackage(config.getParent(), config.getVo()));
        packageInfo.put(ConstVal.DETAIL, joinPackage(config.getParent(), config.getVo()));
        ...
        if (StringUtils.isNotEmpty(template.getXml())) {
            pathInfo.put(ConstVal.XML_PATH, joinPath(outputXmlDir + File.separator + "mapper", config.getModuleName()));
        }
        if (StringUtils.isNotEmpty(template.getEntityModel())) {
            if (StringUtils.isNotEmpty(outputClientDir)) {
                pathInfo.put(ConstVal.ENTITY_MODEL_PATH, joinPath(outputClientDir, packageInfo.get(ConstVal.MODEL)));
            } else {
                pathInfo.put(ConstVal.ENTITY_MODEL_PATH, joinPath(outputDir, packageInfo.get(ConstVal.MODEL)));
            }

        }
        if (StringUtils.isNotEmpty(template.getParamsModel())) {
            if (StringUtils.isNotEmpty(outputClientDir)) {
                pathInfo.put(ConstVal.PARAMS_MODEL_PATH, joinPath(outputClientDir, packageInfo.get(ConstVal.PARAMS)));
            } else {
                pathInfo.put(ConstVal.PARAMS_MODEL_PATH, joinPath(outputDir, packageInfo.get(ConstVal.PARAMS)));
            }
        }
        if (StringUtils.isNotEmpty(template.getAddParamsModel())) {
            if (StringUtils.isNotEmpty(outputClientDir)) {
                pathInfo.put(ConstVal.ADD_PARAMS_MODEL_PATH, joinPath(outputClientDir, packageInfo.get(ConstVal.ADD_PARAMS)));
            } else {
                pathInfo.put(ConstVal.ADD_PARAMS_MODEL_PATH, joinPath(outputDir, packageInfo.get(ConstVal.ADD_PARAMS)));
            }
        }

        if (StringUtils.isNotEmpty(template.getDetailModel())) {
            if (StringUtils.isNotEmpty(outputClientDir)) {
                pathInfo.put(ConstVal.DETAIL_MODEL_PATH, joinPath(outputClientDir, packageInfo.get(ConstVal.DETAIL)));
            } else {
                pathInfo.put(ConstVal.DETAIL_MODEL_PATH, joinPath(outputDir, packageInfo.get(ConstVal.DETAIL)));
            }
        }
        if (StringUtils.isNotEmpty(template.getClient()) && StringUtils.isNotEmpty(outputClientDir)) {
            pathInfo.put(ConstVal.CLIENT_PATH, joinPath(outputClientDir, packageInfo.get(ConstVal.CLIENT)));
        }
        ...
        //Mapper 类引用的统一报
        mapperClassPackage = new ArrayList<>();
        mapperClassPackage.add(MyBatisPlusPackageVal.Param());
        mapperClassPackage.add(MyBatisPlusPackageVal.Page());


        //Service 类引用的统一报
        serviceClassPackage = new ArrayList<>();
        //ServiceImpl 类引用的统一报
        serviceImplClassPackage = new ArrayList<>();
        serviceImplClassPackage.add(MyBatisPlusPackageVal.Page());
    }
    ...
    private List<TableInfo> processTable(List<TableInfo> tableList, NamingStrategy strategy, StrategyConfig config) {
        ...
        for (TableInfo tableInfo : tableList) {
            tableInfo.setOriginalName(NamingStrategy.capitalFirst(processName(tableInfo.getName(), strategy, tablePrefix)));
            tableInfo.setCapitalFirstName(NamingStrategy.capitalFirst(processName2(tableInfo.getName(), strategy, tablePrefix)));
            tableInfo.setEntityName(strategyConfig, NamingStrategy.capitalFirst(processName(tableInfo.getName(), strategy, tablePrefix) + ConstVal.ENTITY));
            String originalName = tableInfo.getOriginalName();
            tableInfo.setEntityModel(originalName + "ListModel");
            tableInfo.setParamsModel(originalName + "ListParams");
            if (originalName.endsWith("Detail")) {
                tableInfo.setDetailModel(originalName + "Model");
            } else {
                tableInfo.setDetailModel(originalName + "DetailModel");
            }

            tableInfo.setAddParamsModel(originalName + "AddOrUpdateParams");
            tableInfo.setClient(originalName + "Client");
            if (StringUtils.isNotEmpty(globalConfig.getMapperName())) {
                tableInfo.setMapperName(String.format(globalConfig.getMapperName(), tableInfo.getOriginalName()));
            } else {
                tableInfo.setMapperName(originalName + ConstVal.MAPPER);
            }
            if (StringUtils.isNotEmpty(globalConfig.getXmlName())) {
                tableInfo.setXmlName(String.format(globalConfig.getXmlName(), originalName));
            } else {
                tableInfo.setXmlName(originalName + ConstVal.MAPPER);
            }
            if (StringUtils.isNotEmpty(globalConfig.getServiceName())) {
                tableInfo.setServiceName(String.format(globalConfig.getServiceName(), originalName));
            } else {
                tableInfo.setServiceName("I" + originalName + ConstVal.SERIVCE);
            }


            if (StringUtils.isNotEmpty(globalConfig.getServiceImplName())) {
                tableInfo.setServiceImplName(String.format(globalConfig.getServiceImplName(), originalName));
            } else {
                tableInfo.setServiceImplName(originalName + ConstVal.SERVICEIMPL);
            }
            if (StringUtils.isNotEmpty(globalConfig.getControllerName())) {
                tableInfo.setControllerName(String.format(globalConfig.getControllerName(), tableInfo.getClient()));
            } else {
                tableInfo.setControllerName(originalName + ConstVal.CONTROLLER);
            }
            //强制开启字段注解
    ...
    private void checkTableIdTableFieldAnnotation(StrategyConfig config, TableInfo tableInfo, String[] fieldPrefix) {
        ...
        if (importTableFieldAnnotaion) {
            tableInfo.getImportPackages().add(MyBatisPlusPackageVal.TableField());
        }
        if (importTableIdAnnotaion) {
            tableInfo.getImportPackages().add(MyBatisPlusPackageVal.TableId());
        }
        if (globalConfig.getIdType() != null) {
            if (!importTableIdAnnotaion) {
                tableInfo.getImportPackages().add(MyBatisPlusPackageVal.TableId());
            }
            tableInfo.getImportPackages().add(MyBatisPlusPackageVal.IdType());
        }
    }
    ...
    private TableInfo convertTableFields(TableInfo tableInfo, NamingStrategy strategy) {
        ...
        try {
            ...
            while (results.next()) {
                ...
                boolean isId = StringUtils.isNotEmpty(key) && "PRI".equalsIgnoreCase(key);
                // 处理ID
                if (isId && !haveId) {
     ...
     * @return 根据策略返回处理后的名称
     */
    private String processName2(String name, NamingStrategy strategy, String[] prefix) {
        boolean removePrefix = false;
        if (prefix != null && prefix.length >= 1) {
            removePrefix = true;
        }
        ...
        if (removePrefix) {
            if (strategy == NamingStrategy.underline_to_camel) {
                // 删除前缀、下划线转驼峰
                propertyName = NamingStrategy.removePrefixAndCamel2(name, prefix);
            } else {
                // 删除前缀
                propertyName = NamingStrategy.removePrefix2(name, prefix);
            }
        } else if (strategy == NamingStrategy.underline_to_camel) {
            // 下划线转驼峰
            propertyName = NamingStrategy.underlineToCamel2(name);
        } else {
            // 不处理
            propertyName = name;
        }
