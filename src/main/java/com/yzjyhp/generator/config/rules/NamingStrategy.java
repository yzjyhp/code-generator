...
public enum NamingStrategy {
    ...
    public static String underlineToCamel(String name) {
        // 快速检查
        if (GeneratorStringUtils.isEmpty(name)) {
            // 没必要转换
            return "";
        }
        String tempName = name;
        // 大写数字下划线组成转为小写 , 允许混合模式转为小写
        if (GeneratorStringUtils.isCapitalMode(name) || GeneratorStringUtils.isMixedMode(name)) {
        ...
        for (String camel : camels) {
            ...
            if (GeneratorStringUtils.isEmpty(camel)) {
                continue;
            }
            // 处理真正的驼峰片段
            if (result.length() == 0) {
    ...
    public static String underlineToCamel2(String name) {
        // 快速检查
        if (GeneratorStringUtils.isEmpty(name)) {
            // 没必要转换
            return "";
        ...
        for (String camel : camels) {
            ...
            if (result.length() == 0) {
                ...
                // 其他的驼峰片段，首字母大写
                result.append(capitalFirst(camel)).append(" ");
            }
        }
        if(result.length() > 0){
     ...
     * @return
     */
    public static String removePrefix2(String name, String... prefix) {
        if (GeneratorStringUtils.isEmpty(name)) {
            return "";
        }
        if (null != prefix) {
     ...
     * @return
     */
    public static String removePrefixAndCamel2(String name, String[] tablePrefix) {
        return underlineToCamel2(removePrefix(name, tablePrefix));
    }
    ...
    public static String capitalFirst(String name) {
        if (GeneratorStringUtils.isNotEmpty(name)) {
